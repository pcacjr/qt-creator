...
FilesSelectionWizardPage::FilesSelectionWizardPage(PythonProjectWizardDialog *pythonProjectWizard, QWidget *parent)
    : QWizardPage(parent), m_pythonProjectWizardDialog(pythonProjectWizard), m_model(0), m_finished(false)
{
...
void FilesSelectionWizardPage::initializePage()
{
    ...
    m_model = new SelectableFilesModel(m_pythonProjectWizardDialog->path(), this);
...
void FilesSelectionWizardPage::parsingFinished()
{
    ...
    applyFilter();
    // work around qt
    m_pythonProjectWizardDialog->setTitleFormat(m_pythonProjectWizardDialog->titleFormat());
}
